...
function fetch(url) {
  ...
  return new Bluebird((resolve, reject) => {
    var req = getHttpLib(url).get(url);
    var chunks = [];
...
var getPackageMeta = _.memoize(fetch);
  ...
  return getPackageMeta(name)
    .then(doc => {
      ...
      if (typeof range === 'string') {
        match = semver.valid(doc['dist-tags'][range]);
      } else {
      ...
      return new PackageType(doc.versions[match.version], range);
    });
}
...
var pkgJson = require(path.resolve(process.cwd(), process.argv[2]));
var rootPkg = new PackageType(pkgJson);
